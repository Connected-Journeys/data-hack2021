...
paper_keywords <- keywords_enriched %>% 
  filter(!individual_keywords %in% c("New Zealand",
                                     "roads", 
                                     "transport", 
                                     "road")) %>%
  group_by(report_note_number) %>% 
  summarise(keyword_1 = individual_keywords[1],
            keyword_2 = individual_keywords[2],
            keyword_3 = individual_keywords[3])
...
#   select(report_note_number, individual_keywords) %>% 
#   rename(keyword_1 = individual_keywords)
# 
# # create data frame with 2 columns for paper ID
